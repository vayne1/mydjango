...
def repayment():        #还款
    while True:
        repay_cash = input('请输入您要还款的金额，q退出：')
        if repay_cash.isdigit():
            user_dict[username]['salary'] += int(repay_cash)
            print('还款成功，请继续操作')
            log(username+'\t'+'repayment {},success'.format(int(repay_cash)))
        elif repay_cash == 'q':break
        else:print('无效的输入')


@auth
def operation_record():     #操作记录查询
    print('以下为您的历史操作记录：')
    print('\033[35;1m*\033[0m' * 50)
    with open('atm.log','r',encoding='utf-8') as f:
        for i in f:
            name = i.strip().split()[3]
            if username == name:print(i.strip())
    print('\033[35;1m*\033[0m' * 50)

@auth
def account_manage():       #账号管理
    if username == 'admin':
        option_list = ['添加账户','额度管理','冻结账户']
        while True:
            print('\033[35;1m*\033[0m' * 50)
            for i in option_list:
                print(str(option_list.index(i))+'.'+i)
            print('\033[35;1m*\033[0m' * 50)
            user_choose = input('请输入您要操作选项的序号，q退出：')
            if user_choose.isdigit() and int(user_choose) in range(len(option_list)):
                if int(user_choose.strip()) == 0:   #添加账户
                    user_create = input('请输入用户名密码，初始信用额度为15000，以"/"作为分隔符：')
                    if user_create.strip().find('/') != -1:
                        account_list = user_create.strip().split('/')
                        user_dict[account_list[0]] = {'pwd':account_list[1],'status':0,'salary':15000,'amount':15000}
                        log(username+'\t'+'create account {} success'.format(account_list[0]))
                        print('\033[32;1m添加成功\033[0m')
                    else:print('\033[31;1m输入格式有误\033[0m')
                if int(user_choose) == 1:       #额度管理
                    cash_user = input('输入您要修改额度的账户：')
                    if user_dict.get(cash_user):
                        cash_amount = input('输入要修改的金额：')
                        if cash_amount.isdigit():
                            user_dict[cash_user]['salary'] =user_dict[cash_user]['salary']+int(cash_amount)-user_dict[cash_user]['amount']      #修改剩余额度金额
                            user_dict[cash_user]['amount'] = int(cash_amount)   #修改总额度
                            print('额度修改成功')
                            log(username+'\t'+'modify {} amount success'.format(cash_user))
                        else:print('无效的输入')
                    else:print('无此用户')
                if int(user_choose) == 2:       #冻结账户
                    frozen = input('输入您要冻结的账户：')
                    if user_dict.get(frozen):
                        user_dict[frozen]['status'] = 3
                        print(user_dict[frozen]['status'])
                        print('账号{}成功被冻结'.format(frozen))
                        log(username+'\t'+'frozen {} success'.format(frozen))
                    else:print('无此用户')
            elif user_choose == 'q':break
            else:print('无效的输入')
...
def main_func():
    menu = {
        ...
        'ATM': {
            ...
            '转账': {'func':'transfer()'},
            '还款': {'func':'repayment()'},
            '操作记录': {'func':'operation_record()'},
            '账户管理': {'func':'account_manage()'},
        },
    }
    floor = menu  # 当前层
    status_floor = []
